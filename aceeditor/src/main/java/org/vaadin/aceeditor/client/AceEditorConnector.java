...
public class AceEditorConnector extends AbstractHasComponentsConnector
  ...
  protected AceEditorServerRpc serverRpc = RpcProxy.create(AceEditorServerRpc.class, this);
  ...
  protected class SendTimer extends Timer {
    ...
    public void schedule(final int ms, final SendCond send) {
      super.schedule(ms);
      this.send = this.send.or(send);
      scheduled = true;
    }

    public void scheduleIfNotAlready(final int ms, final SendCond send) {
      if (!scheduled) {
        schedule(ms, send);
      }
    }
  ...
  protected enum SendCond {
    ...
    public SendCond or(final SendCond sw2) {
      return ordinal() > sw2.ordinal() ? this : sw2;
    }
  }
  ...
  protected AceEditorClientRpc clientRpc = new AceEditorClientRpc() {
    ...
    public void diff(final TransportDiff ad) {
      ...
      final AceDoc doc1 = getWidget().getDoc();
  ...
  public void onStateChanged(final StateChangeEvent stateChangeEvent) {
    ...
    final boolean firstTime = !getWidget().isInitialized();
    if (firstTime) {
    ...
    if (getState().selection != null) {
      final AceRange sel = AceRange.fromTransport(getState().selection);
      if (firstTime) {
  ...
  protected void sendToServer(final SendCond send, final boolean immediately) {
    ...
    final AceDoc doc = getWidget().getDoc();
    final ClientSideDocDiff diff = ClientSideDocDiff.diff(shadow, doc);
